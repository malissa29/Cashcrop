...
$username=clean(strtolower(strip_tags($_POST['username'])));
...
$password=clean(strip_tags($_POST['pswd']));
	...
	if($username  && $password)
	{
		...
		if($username)
		{
			$query=mysqli_query($conn,"SELECT * FROM cc_m_customer WHERE cust_uname='$username'");
			$query1=mysqli_query($conn,"SELECT * FROM cc_m_customer_details WHERE cust_uname='$username'");
		...
		if($numrows!=0)
		{
			while($row=mysqli_fetch_assoc($query))
			{
				...
				$dbusername=$row['cust_uname'];
				$dbpassword=$row['cust_pswd'];
				$dbuser=$row1['cust_fname'];
...
$phone=clean(strip_tags($_POST['phone']));
		...
		db_connect();
		$query="SELECT cust_uname FROM cc_m_customer WHERE cust_uname='$username1'";
		//echo $query;
		$namecheck=mysqli_query($conn,$query);
		...
		{	if(strlen($password1)>6 && strlen($password1)<26){
			if($password1==$confirmpassword)
				...
				$sql = "INSERT INTO `cc_m_customer_details` ('cust_address','cust_city','cust_state','cust_pincode','cust_uname','cust_fname','cust_lname','cust_email','cust_phone')
				VALUES ('$address, '$city','$state','$pin','$username1','$fname','$lname','$email1','$phone');";
				
				$sql1=  "INSERT INTO `cc_m_customer` ('cust_uname','cust_pswd')
