...
$password=clean(strip_tags($_POST['password']));
	...
	//echo $username;
		...
		if($username)
		{
			$query=mysqli_query($conn,"SELECT * FROM cc_m_admin WHERE a_uname='$username'");
			$query2=mysqli_query($conn,"SELECT * FROM cc_m_admin_details WHERE a_uname='$username'");
		...
		$numrows=mysqli_num_rows(mysqli_query($conn,"SELECT * FROM cc_m_admin WHERE a_uname='$username'"));
		...
		if($numrows!=0)
		{
			while($row=mysqli_fetch_assoc($query))
			{
				...
				$dbusername=$row['a_uname'];
				$dbpassword=$row['a_pswd'];
				$row2=mysqli_fetch_assoc($query2);
				$dbuser=$row2['a_fname'];
		...
		$_SESSION['user']=$dbusername;
				...
				{echo $_SESSION['user'].$_SESSION['username']." You have Successfully logged in";
