...
if (isset($_GET['order_id'])) {
	...
	if(isset($_GET['ddate']))
	{
		...
		if($_GET['ddate']!='Select the Delivery Date')
		{
		
		delivered_product($_GET['order_id'],$_GET['ddate']);
		}
	}
}
...
<body>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					...
					<?php 
							...
							$query="SELECT C.* FROM cc_m_order C where order_status='In Transit'";
						...
						{?>	
						 <tr>
							  ...
							  <div class="controls">
								...
								<input type="text" class="input-xlarge datepicker" id="date01" name='ddate' Value="Select the Delivery Date">
								<input type="hidden" class="input-xlarge datepicker" id="order_id" name='order_id' Value="<?php echo $row['order_id']; ?>">
								
							  &nbsp;
									<button class="btn  btn-danger" type='submit' value='Delivered'>
										<i>Delivered</i>
	...
	<!-- end: JavaScript-->
	<?php
function delivered_product($order_id,$ddate)
	{
		
		
		include 'db_connect.php';
		$newdate=date('Y-m-d',strtotime($ddate));
		$query="UPDATE cc_m_order SET order_status='Delivered',delivery_date='$newdate' WHERE order_id=$order_id";
		//echo $query;
